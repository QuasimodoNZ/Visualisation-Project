...
def create_house(row):
    home = {}
    for col, val in enumerate(row):
        if col == 0 or col == 6:
            continue

        column_details = housing_columns[col]
        ...
        if column_details[1] and len(val) == 0:
            ...
            puma = get_or_create_PUMA(int(val), int(row[col + 2]))
            home['PUMA_id'] = puma.id
        elif column_details[2]:
            home[column_details[0]] = val
        else:
            try:
                home[column_details[0]] = int(val)
            except ValueError as err:
                print 'column for error: {}'.format(col)
                raise
    cols = ''
    vals = ''
    for col, val in home.items():
        cols = cols + col + ', '
        if val == None:
            val = ''
        vals = vals + str(val) + ', '
    return 'INSERT INTO d3js_visualiser_house ({}) VALUES ({})'.format(cols[:-2], vals[:-2])
...
if __name__ == '__main__':
    ...
    freeze_support()
    chunks = []
    with open('D:\pums\csv_hus\hus.csv', 'r') as input:
        input.next()
        data = []
        for row in input:
            data.append(row)
    ...
    print 'loaded {} chunks from the data'.format(number_of_chunks)
